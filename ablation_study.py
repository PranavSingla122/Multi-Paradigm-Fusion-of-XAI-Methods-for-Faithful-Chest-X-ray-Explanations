...
class BaselineArchitectures:
    @staticmethod
    def get_resnet():
        return models.resnet152(pretrained=False, num_classes=Config.NUM_CLASSES)
...
class AblationStudy:
    ...
    def compute_auroc(self, model: nn.Module, loader) -> float:
        ...
        try:
            ...
            return float(roc_auc_score(all_labels, all_probs, multi_class='ovr', average='macro'))
    ...
    def run_complete_ablation_study(self, num_trials: int = None, epochs: int = None) -> Dict:
        ...
        print("\n" + "="*80)
        print("BASELINE ARCHITECTURE COMPARISON")
        print("="*80)
        print(f"Epochs: {epochs} | Trials: {num_trials}")
        ...
        # Baselines — same epochs for fair comparison
    ...
    def _perform_statistical_comparison(self, results: Dict) -> Dict:
        ...
        for name, res in results.items():
            if name != 'statistical_comparison' and isinstance(res, dict) and 'accuracy' in res:
    ...
    def _save_results(self, results: Dict):
        ...
        path = os.path.join(Config.ABLATION_DIR, 'baseline_comparison_results.json')
        with open(path, 'w') as f:
            json.dump(results, f, indent=4, default=str)
        print(f"\nResults saved: {path}")

    def _generate_ablation_report(self, results: Dict):
        report  = "# Baseline Architecture Comparison\n\n"
        report += f"**Epochs**: 50 (same for all models)\n\n"
        report += "## Summary\n\n"
        ...
        path = os.path.join(Config.ABLATION_DIR, 'ablation_report.md')
        with open(path, 'w') as f:
            f.write(report)
