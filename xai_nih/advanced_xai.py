...
log_filename = os.path.join(log_dir, f"xai_optimized_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
logging.basicConfig(
    level=logging.INFO,
...
MODEL_PATH = '/teamspace/studios/this_studio/xai_nih/outputs/models/best_model_auroc.pth'
MEAN = np.array([0.485, 0.456, 0.406])
STD  = np.array([0.229, 0.224, 0.225])


def load_model(model_path=MODEL_PATH, num_classes=14, device='cuda'):
    try:
...
class NIHBBoxLoader:
    def __init__(self, bbox_csv_path: str, image_size: int = 224, original_size: int = 1024):
        self.image_size = image_size
        self.scale = image_size / original_size
        self.bbox_data = {}
        if os.path.exists(bbox_csv_path):
            df = pd.read_csv(bbox_csv_path)
            for _, row in df.iterrows():
                fname = row['Image Index']
                if fname not in self.bbox_data:
                    self.bbox_data[fname] = []
                x = int(row.iloc[2] * self.scale)
                y = int(row.iloc[3] * self.scale)
                w = int(row.iloc[4] * self.scale)
                h = int(row.iloc[5] * self.scale)
                self.bbox_data[fname].append({
                    'label': row.iloc[1],
                    'x': max(0, x), 'y': max(0, y),
                    'w': min(w, image_size - max(0, x)),
                    'h': min(h, image_size - max(0, y))
                })
            logger.info(f"Loaded bboxes for {len(self.bbox_data)} images")

    def get_mask(self, image_name: str, target_label: str = None) -> Optional[np.ndarray]:
        if image_name not in self.bbox_data:
            return None
        mask = np.zeros((self.image_size, self.image_size), dtype=np.float32)
        for box in self.bbox_data[image_name]:
            if target_label and box['label'] != target_label:
                continue
            x, y, w, h = box['x'], box['y'], box['w'], box['h']
            mask[y:y+h, x:x+w] = 1.0
        return mask if mask.max() > 0 else None

    def get_bboxes(self, image_name: str) -> List[Dict]:
        return self.bbox_data.get(image_name, [])
...
class PyramidXAIFusion:
    ...
    def pyramid_fusion(self, gradcam, shap, lime, weights=None):
        ...
        for scale, sw in zip(scales, weights):
            ...
            fused.append(sw * cv2.resize(f, (224, 224)))
...
class EnhancedXAIExplainer:
    ...
    def __init__(self, model, device=None, bbox_csv_path=None):
        self.model  = model
        ...
        self.gt_metrics  = GroundTruthMetrics()
        if bbox_csv_path and os.path.exists(bbox_csv_path):
            self.bbox_loader = NIHBBoxLoader(bbox_csv_path)
    ...
    def get_gradcam_explanations(self, images, target_layers=None, method='gradcam') -> List[np.ndarray]:
    ...
    def get_gradcam_plus_plus(self, images) -> np.ndarray:
        ...
        try:
            ...
            grad_2        = grad ** 2
            grad_3        = grad ** 3
            sum_act_grad3 = (act * grad_3).sum(dim=(2, 3), keepdim=True)
            alpha_denom   = 2.0 * grad_2 + sum_act_grad3 + 1e-8
            alpha         = grad_2 / alpha_denom              # FIX: was missing
            alpha         = alpha * (grad > 0).float()
    ...
    def get_integrated_gradients(self, images, steps: int = 64) -> np.ndarray:
        ...
        blurred = cv2.GaussianBlur((img_np * 255).astype(np.uint8), (51, 51), 20)
        baseline = torch.from_numpy(blurred.astype(np.float32) / 255.0).permute(2, 0, 1).unsqueeze(0).to(self.device).float()
        ...
        saliency = cv2.GaussianBlur(saliency.astype(np.float32), (11, 11), 3.0)
    ...
    def get_rise_explanations(self, images, num_masks: int = 8000, p: float = 0.4,
                              mask_size: int = 10, batch_size: int = 64) -> np.ndarray:
        if len(images.shape) == 3:
            images = images.unsqueeze(0)
        images = images.to(self.device)
        self.model.eval()
    ...
    def get_shap_explanations(self, images, **kwargs) -> np.ndarray:
        ...
        n_steps  = 20
        grad_acc = torch.zeros(1, 3, 224, 224, device=self.device)
    ...
    def faithfulness_insertion_deletion(self, image, explanation, num_steps=100):
        ...
        for step in range(num_steps + 1):
            ...
            mt   = torch.from_numpy(mask.reshape(224, 224)).float().to(self.device).unsqueeze(0).unsqueeze(0)
        ...
        for _ in range(num_perturbations):
            ...
            try:
                if method_name == 'GradCAM':
                    p_exp = self.pyramid_fusion.normalize(self._ensure_224(self.get_gradcam_explanations(perturbed)[0]))
                elif method_name == 'GradCAM++':
                    p_exp = self.pyramid_fusion.normalize(self._ensure_224(self.get_gradcam_plus_plus(perturbed)))
                elif method_name == 'IntegratedGradients':
                    p_exp = self.pyramid_fusion.normalize(self._ensure_224(self.get_integrated_gradients(perturbed)))
                elif method_name == 'RISE':
                    p_exp = self.pyramid_fusion.normalize(self._ensure_224(self.get_rise_explanations(perturbed, num_masks=2000, p=0.5, mask_size=8)))
                elif method_name == 'LIME':
                    p_exp = self.pyramid_fusion.normalize(self._ensure_224(self.get_lime_explanations(perturbed)))
                    ...
                    gc = self.pyramid_fusion.normalize(self._ensure_224(self.get_gradcam_explanations(perturbed)[0]))
                    li = self.pyramid_fusion.normalize(self._ensure_224(self.get_lime_explanations(perturbed)))
                    sh = self.pyramid_fusion.normalize(self._ensure_224(shap_map))
    ...
    def visualize_all_methods(self, image, save_dir='visualizations', sample_id=0,
                               class_names=None, gt_mask=None, shap_map=None):
        ...
        gradcam    = self.pyramid_fusion.normalize(self._ensure_224(self.get_gradcam_explanations(image)[0])).astype(np.float32)
        gradcam_pp = self.pyramid_fusion.normalize(self._ensure_224(self.get_gradcam_plus_plus(image))).astype(np.float32)
        ig         = self.pyramid_fusion.normalize(self._ensure_224(self.get_integrated_gradients(image))).astype(np.float32)
        rise       = self.pyramid_fusion.normalize(self._ensure_224(self.get_rise_explanations(image))).astype(np.float32)
        lime_map   = self.pyramid_fusion.normalize(self._ensure_224(self.get_lime_explanations(image))).astype(np.float32)
        shap_vis   = self.pyramid_fusion.normalize(self._ensure_224(
            shap_map if shap_map is not None else self.get_shap_explanations(image)
        )).astype(np.float32)
        pyramid   = self.pyramid_fusion.pyramid_fusion(gradcam, shap_vis, lime_map).astype(np.float32)
        attention = self.pyramid_fusion.attention_fusion(gradcam, shap_vis, lime_map).astype(np.float32)

        has_gt = gt_mask is not None and gt_mask.max() > 0
        n_rows = 3 if has_gt else 2
        fig    = plt.figure(figsize=(28, 5 * n_rows))
        gs_fig = fig.add_gridspec(n_rows, 5, hspace=0.3, wspace=0.3)
        ...
        for c, (hm, lbl) in enumerate([(lime_map, 'LIME'), (pyramid, 'Pyramid Fusion'), (attention, 'Attention Fusion')]):
        ...
        if has_gt:
            ...
            ax.imshow(img_np); ax.imshow(gt_mask, cmap='Reds', alpha=0.4)
            ax.set_title('Ground Truth', fontsize=12, fontweight='bold'); ax.axis('off')
            for ci, (mn, exp) in enumerate([('GradCAM', gradcam), ('GradCAM++', gradcam_pp),
                                             ('IG', ig), ('Pyramid', pyramid)]):
                ax = fig.add_subplot(gs_fig[2, ci + 1])
                ax.imshow(img_np); ax.imshow(exp, cmap='jet', alpha=0.45, vmin=0, vmax=1)
                contours, _ = cv2.findContours(gt_mask.astype(np.uint8), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
                for cnt in contours:
                    pts = cnt.squeeze()
                    if len(pts.shape) == 2:
                        ax.plot(np.append(pts[:, 0], pts[0, 0]), np.append(pts[:, 1], pts[0, 1]), 'lime', lw=2)
                iou = GroundTruthMetrics.compute_iou(exp, gt_mask)
                pg  = GroundTruthMetrics.pointing_game(exp, gt_mask)
                ax.set_title(f'{mn}\nIoU={iou:.3f} PG={pg:.0f}', fontsize=11); ax.axis('off')
    ...
    def visualize_comparison_grid(self, images, save_dir='visualizations', class_names=None, max_samples=9):
        ...
        for idx in range(n):
            ...
            gc_  = self.pyramid_fusion.normalize(self._ensure_224(self.get_gradcam_explanations(img)[0])).astype(np.float32)
            gcpp = self.pyramid_fusion.normalize(self._ensure_224(self.get_gradcam_plus_plus(img))).astype(np.float32)
            ig_  = self.pyramid_fusion.normalize(self._ensure_224(self.get_integrated_gradients(img))).astype(np.float32)
            rise_= self.pyramid_fusion.normalize(self._ensure_224(self.get_rise_explanations(img))).astype(np.float32)
            li_  = self.pyramid_fusion.normalize(self._ensure_224(self.get_lime_explanations(img))).astype(np.float32)
            sh_  = self.pyramid_fusion.normalize(self._ensure_224(self.get_shap_explanations(img))).astype(np.float32)
    ...
    @staticmethod
    def _ensure_224(arr: np.ndarray) -> np.ndarray:
        if arr.ndim == 3:
            arr = arr.mean(axis=2)
        if arr.shape != (224, 224):
            arr = cv2.resize(arr.astype(np.float32), (224, 224), interpolation=cv2.INTER_CUBIC)
        return arr.astype(np.float32)

    def evaluate_xai_with_pyramid_fusion(
        self, test_loader, num_samples=300, target_layers=None,
        train_loader=None, batch_size=16, visualize_samples=True,
        vis_save_dir='visualizations', class_names=None,
        image_names=None, run_dynamic_layer_selection=False
    ):
        logger.info("=" * 60)
        logger.info("NIH XAI EVALUATION")
        ...
        for _, (images, _) in enumerate(test_loader):
            ...
            for i in range(images.size(0)):
                ...
                gt_mask  = None
                img_name = None
                if image_names is not None and name_idx < len(image_names):
                    img_name = image_names[name_idx]
                    if self.bbox_loader:
                        gt_mask = self.bbox_loader.get_mask(img_name)
                name_idx += 1

                try:
                    gradcam    = self._ensure_224(self.get_gradcam_explanations(image)[0])
                    gradcam_pp = self._ensure_224(self.get_gradcam_plus_plus(image))
                    ig         = self._ensure_224(self.get_integrated_gradients(image))
                    rise       = self._ensure_224(self.get_rise_explanations(image))
                    lime_map   = self._ensure_224(self.get_lime_explanations(image))
                    ...
                    shap_map = self.pyramid_fusion.normalize(
                        self._ensure_224(self.get_shap_explanations(image))
                    ).astype(np.float32)
                    ...
                    exp_data = {
                        ...
                        'gt_mask':    gt_mask, 'image_name': img_name
                    }
                    ...
                    if visualize_samples and collected <= 3:
                        self.visualize_all_methods(
                            ...
                            class_names=class_names, gt_mask=gt_mask, shap_map=shap_map
                        )
                except Exception as e:
        ...
        for method_name, key in method_configs.items():
            ...
            for idx, ed in enumerate(all_explanations):
                try:
                    ...
                    if ed['gt_mask'] is not None:
                        exp, gt = ed[key], ed['gt_mask']
                        results[method_name]['iou'].append(GroundTruthMetrics.compute_iou(exp, gt))
                        results[method_name]['pointing_game'].append(GroundTruthMetrics.pointing_game(exp, gt))
                        results[method_name]['energy_bbox'].append(GroundTruthMetrics.energy_inside_bbox(exp, gt))
                    if (idx + 1) % 20 == 0:
        ...
        self._print_results_nih(final)
        if visualize_samples:
            self._plot_results_nih(final, save_dir=vis_save_dir)
            if stat_results:
    ...
    def _print_results_nih(self, final):
        sep = "=" * 160
        print(f"\n{sep}")
        print("NIH XAI RESULTS".center(160))
        print(sep)

        has_gt = any(isinstance(final.get(m), dict) and 'iou' in final[m]
                     for m in ['GradCAM', 'GradCAM++', 'Pyramid-Fusion'])

        if has_gt:
            hdr = (f"{'Method':<22} {'Insertion ^':<18} {'Deletion':<18} "
                   f"{'SSIM ^':<18} {'RankCorr ^':<18} {'IoU ^':<16} {'PG ^':<16} {'Energy ^':<16}")
        else:
        ...
        for mn in ['GradCAM', 'GradCAM++', 'IntegratedGradients', 'RISE', 'LIME', 'Pyramid-Fusion', 'Attention-Fusion']:
            ...
            if has_gt:
                for k in ['iou', 'pointing_game', 'energy_bbox']:
                    row += f" {m.get(k,0):.4f}±{m.get(f'{k}_std',0):.4f}  "
            print(row)

        print(sep)
        print(f"Best Method: {final.get('best_method','N/A')} | Total: {final.get('total_time_minutes',0):.1f}min | "
              f"CAM: {final.get('cam_target_layer','N/A')}".center(160))

        if final.get('statistical_tests'):
            print("\n" + "STATISTICAL SIGNIFICANCE".center(160))
    ...
    def _plot_results_nih(self, results, save_dir='visualizations'):
        os.makedirs(save_dir, exist_ok=True)
        methods = ['GradCAM', 'GradCAM++', 'IntegratedGradients', 'RISE', 'LIME', 'Pyramid-Fusion', 'Attention-Fusion']
        core    = ['insertion_auc', 'deletion_auc', 'robustness_ssim', 'robustness_rank_corr']
        has_gt  = any(isinstance(results.get(m), dict) and 'iou' in results[m] for m in methods)
        if has_gt:
            core += ['iou', 'pointing_game', 'energy_bbox']
        ...
        titles = {
            ...
            'iou':                  ('IoU@0.5',          'darkorange'),
            'pointing_game':        ('Pointing Game',    'mediumpurple'),
            'energy_bbox':          ('Energy BBox',      'teal')
        }
        fig, axes = plt.subplots(1, len(core), figsize=(6 * len(core), 6))
        if len(core) == 1: axes = [axes]
        x = np.arange(len(methods))

        for ai, metric in enumerate(core):
            ax    = axes[ai]
            ...
            lbl, clr = titles.get(metric, (metric, 'gray'))
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, 'nih_results_comparison.png'), dpi=300, bbox_inches='tight')
        plt.close()
...
def main():
    ...
    bbox_csv = '/root/.cache/kagglehub/datasets/nih-chest-xrays/data/versions/3/BBox_List_2017.csv'

    explainer = EnhancedXAIExplainer(
        model=model, device=device,
        bbox_csv_path=bbox_csv if os.path.exists(bbox_csv) else None
    )
    results = explainer.evaluate_xai_with_pyramid_fusion(
        test_loader=test_loader, num_samples=100,
        train_loader=train_loader, batch_size=16,
