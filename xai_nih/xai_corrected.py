...
def get_gradcam_plus_plus(model, image: torch.Tensor,
    ...
    try:
        ...
        # FIX: denominator must be sum(act * grad^3) over spatial dims,
        # NOT sum(act) * grad^3. The original bug flattened alphas to ~0,
        # causing uniform weights and a diffuse full-image heatmap.
        sum_act_grad3 = (act * grad_3).sum(dim=(2, 3), keepdim=True)
        alpha_denom   = 2.0 * grad_2 + sum_act_grad3 + 1e-8
        alpha   = grad_2 / alpha_denom
        # Only positive gradients contribute (per paper)
...
def get_integrated_gradients(model, image: torch.Tensor,
    ...
    baseline = torch.from_numpy(blurred.astype(np.float32) / 255.0).permute(2, 0, 1).unsqueeze(0).to(device)
